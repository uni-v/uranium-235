  ...
  /* Main and Proc_0 in the Ada version             */
{
  ...
  //if (User_Time < Too_Small_Time * freq())
  if (0)
  {
  ...
  {
#ifdef TIME
    Microseconds = (float) User_Time / freq() * Mic_secs_Per_Second 
                        / (float) Number_Of_Runs;
    Dhrystones_Per_Second = (float) Number_Of_Runs / ((float) User_Time / freq());
#else
    Microseconds = (float) User_Time / freq() * Mic_secs_Per_Second 
                        / ((float) HZ * ((float) Number_Of_Runs));
    Dhrystones_Per_Second = ((float) HZ * (float) Number_Of_Runs)
                        / ((float) User_Time / freq());
#endif
    ...
    //DMIPS_mul = DMIPS_MHZ * 10000;
    //printf("DMIPS_mul = %d\n", DMIPS_mul);
    //DMIPS_int = DMIPS_mul / 10000;
    //DMIPS_dec = DMIPS_mul % 10000;

    //printf ("Print Personal Added Addtional Info to Easy Visual Analysis\n");
    //printf ("\n");
    printf (" (*) User_Cycle for total run through Dhrystone with loops %d: ", Number_Of_Runs);
    printf ("%d \n", User_Cycle);
    ...
    printf ("\n");
    printf (" (*) The DMIPS/MHz can be calculated by: \n");
    //printf ("       1000000/(User_Cycle/Number_Of_Runs)/1757 = %d.04%d DMIPS/MHz\n", DMIPS_int, DMIPS_dec);
