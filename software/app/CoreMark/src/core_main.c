...
#error "Please define a way to initialize a memory block."
	...
	printf("Core Mark Started: Iterations = %d\n", results[0].iterations);
	start_time();
#if (MULTITHREAD>1)
	...
	if (known_id>=0) {
		for (i=0 ; i<default_num_contexts; i++) {
			...
			if ((results[i].execs & ID_LIST) && 
				...
				printf("results[%d].crclist = %d, %x\n", i, results[i].crclist, &results[i].crclist);
				printf("list_known_crc[%d] = %d, %x\n", known_id, list_known_crc[known_id], &list_known_crc[known_id]);
				results[i].err++;
			}
			if ((results[i].execs & ID_MATRIX) &&
				(results[i].crcmatrix!=matrix_known_crc[known_id])) {
				ee_printf("[%u]ERROR! matrix crc 0x%04x - should be 0x%04x\n",i,results[i].crcmatrix,matrix_known_crc[known_id]);
				printf("results[%d].crcmatrix = %d, %x\n", i, results[i].crcmatrix, &results[i].crcmatrix);
				printf("matrix_known_crc[%d] = %d, %x\n", known_id, matrix_known_crc[known_id], &matrix_known_crc[known_id]);
				results[i].err++;
			}
			if ((results[i].execs & ID_STATE) &&
				(results[i].crcstate!=state_known_crc[known_id])) {
				ee_printf("[%u]ERROR! state crc 0x%04x - should be 0x%04x\n",i,results[i].crcstate,state_known_crc[known_id]);
				printf("results[%d].crcstate = %d, %x\n", i, results[i].crcstate, &results[i].crcstate);
				printf("state_known_crc[%d] = %d, %x\n", known_id, state_known_crc[known_id], &state_known_crc[known_id]);
				results[i].err++;
			}
	...
	//unsigned long coremark_mul = coremark_dmips * 10000;
	//unsigned long coremark_int = coremark_mul / 10000;
	//unsigned long coremark_dec = coremark_mul % 10000;
	//ee_printf("coremark_mul = %d\n", coremark_mul);
    ...
    //ee_printf ("     (Iterations*1000000/total_ticks) = %1d.%04d CoreMark/MHz\n", coremark_int, coremark_dec);
